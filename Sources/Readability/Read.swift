...
//         }
    ...
    private func grabArticle(_ doc: Element? = nil) -> Element? {
        ...
        for pt in nodesToScore {
            ...
            if parentNode != nil {
                if let parent = parentNode, let readabilityAttr = try? parent.attr("readability"), let score = Int(readabilityAttr) { parentCurrentScore = score }
            }
            if grandParentNode != nil {
                if let grandParent = grandParentNode, let readabilityAttr = try? grandParent.attr("readability"), let score = Int(readabilityAttr) { grandParentCurrentScore = score }
            }
